    ...
    public boolean onCommand(CommandSender cs, Command cmd, String string, String[] args) {
        ...
        if(cmd.getName().equalsIgnoreCase("gladiadoradmin")){
            if(cs.hasPermission("hgladiador.staff")){
                if(args.length == 0){
                    cs.sendMessage("§4[HGladiador] Comandos");
                    ...
                    if(vg.isOcorrendo){
                        ...
                        if(kickado == null){
                            ...
                            if(vg.todosParticipantes.contains(kickado)){
                                ...
                                cs.sendMessage("§4[HGladiador] §cPlayer kickado com sucesso");
                            }else{
                                cs.sendMessage("§4[HGladiador] §cEsse jogador nao esta no gladiador");
                            }
                        }
                    }else{
                        cs.sendMessage("§4[HGladiador] §cNao existe nenhum gladiador ocorrendo no momento");
                    ...
                    if(vg.isOcorrendo){
                        ...
                        if(kickado == null){
                            cs.sendMessage("§4[HGladiador] §cClan nao encontrado");
                        }else{
                            for(String s : vg.clans.keySet()){
                                if(s.contains(kickado.getColorTag())){
                                    ...
                                    cs.sendMessage("§4[HGladiador] §cClan kickado com sucesso");
                                }else{
                                    cs.sendMessage("§4[HGladiador] §cEsse clan nao esta no gladiador");
                                }
                            }
                        }
                    }else{
                    ...
                    hg.reloadConfig();
                    cs.sendMessage("§4[HGladiador] §cReload realizado com sucesso!");
                }
            }else{
                cs.sendMessage("§4[HGladiador] §cVoce nao tem permissao para fazer isso");
            }
        }
